...
test("serializes the complete assessment contract", () => {
  ...
  assert.equal(payload.diagnosis.overallDiagnosisType, "EXCELLENT");
  assert.equal(payload.diagnosis.primaryBottleneckImpact, null);
  assert.deepEqual(payload.diagnosis.secondaryBottlenecks, []);
  assert.deepEqual(payload.diagnosis.requiredConversions, []);
